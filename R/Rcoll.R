...
mpi.scatter.Robj2worker=function (obj, comm = 1) {
    if (!is.list(obj))
        stop("Only list object is allowed to scatter to workers.")
    if (length(obj) != (mpi.comm.size(comm)-1)) 
        stop("Length of your list object is not the same as total number of workers.")
    .tmpname <- list(objname=deparse(substitute(obj), width.cutoff = 500))
    mpi.bcast.Robj2worker(.tmpname)
    mpi.bcast.cmd(cmd = .tmpRobj <- mpi.scatter.Robj(comm = 1), 
        rank = 0, comm = comm)
    mpi.scatter.Robj(obj=c(list("manager"),obj), root = 0, comm = comm)
...
mpi.bcast.Robj2worker <- function(obj, comm=1, all=FALSE){
    if (!all){
		...
		manager.objects <-objects(envir=.GlobalEnv)
		obj.num=length(manager.objects)
		if (obj.num)
			for (i in 1:obj.num){
				mpi.bcast.cmd(cmd=.tmpRobj <- mpi.bcast.Robj(comm=1),
                    rank=0, comm=comm)
				mpi.bcast.Robj(list(objname=manager.objects[i], obj=get(manager.objects[i])), 
					rank=0, comm=comm)
				mpi.bcast.cmd(cmd=assign(.tmpRobj$objname,.tmpRobj$obj), rank=0, comm=comm)
			}
	}
...
mpi.bcast.Rfun2worker <- function(comm=1){
	manager.fun <-objects(envir=.GlobalEnv)
	sync.index <- which(lapply(lapply(manager.fun, get), is.function)==1)
	obj.num=length(sync.index)
	if (obj.num)
		for (i in sync.index){
			mpi.bcast.cmd(cmd=.tmpRobj <- mpi.bcast.Robj(comm=1),
                   rank=0, comm=comm)
			mpi.bcast.Robj(list(objname=manager.fun[i], obj=get(manager.fun[i])), 
				rank=0, comm=comm)
			mpi.bcast.cmd(cmd=assign(.tmpRobj$objname,.tmpRobj$obj), rank=0, comm=comm)
		}
}

mpi.bcast.data2worker <- function(obj, comm=1, buffunit=100){
	if (!is.numeric(obj) || (!is.vector(obj) && !is.matrix(obj)))
		return (mpi.bcast.Robj2worker(obj, comm=comm))
		#stop ("Please use mpi.bcast.Robj2worker")
